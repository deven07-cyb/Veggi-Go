...
export default function CategoryList() {
  ...
  const handleEdit = (category: Categories) => {
    navigate(`/category/detail?id=${category.id}`);
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await FetchData(`/categories/delete/${deleteId}`, 'DELETE');
      if (response.status) {
...
const fetchCountries = async (page: number, limit: number) => {
  ...
  try {
    ...
    if (result.status) {
      const sorted = result.data.categories.sort(
        (a, b) => new Date(b.createsAt).getTime() - new Date(a.createsAt).getTime()
      );
  ...
  const searchCountries = (category: Categories, searchTerm: string) => {
    const term = searchTerm.toLowerCase();
    return (
      category.name?.toLowerCase().includes(term) ||
      (typeof category.status === 'boolean' && (category.status ? 'active' : 'inactive').includes(term))
    );
  };

  // CSV Export mapping
  const exportRow = (item: Categories) => ({
    Name: item.name,
    Status: item.status ? "Active" : "Inactive",
    "Created At": new Date(item.createsAt).toLocaleDateString(),
  });
        ...
        <div className="flex items-center justify-left">
          {info.row.original.image ? (
            <img
              ...
              className="h-20 w-30"
              onError={(e) => {
                (e.target as HTMLImageElement).src = '/images/common/no-image-available.svg';
              }}
            />
          ) : (
            <img
  ...
  const addButton = { label: "Add Category", slug: "/category/detail" };
