...
import { BusinessExport } from "../../Types/Business";
...
export default function BusinessList() {
  ...
  const fetchUserBusiness = async (page: number, search = '') => {
    setLoading(true);
    try {
      const result = await FetchData<BusinessUserResponse>(
        "/user/getAllInfo",
        "POST",
        { page, limit: 10, type: "BUSINESS", search: search }
      );
      if (result.status) {
  ...
  //   if (status) return; // 🚫 Do nothing if user is inactive

  //   setDeleteId(id);     // ✅ Store ID for deletion
  ...
  const exportRow = (item: BusinessExport): Record<string, string | number> => ({
    Name: item.name,
    "Email Address": item.emailAddress,
    Gender: item.gender ?? "N/A",
    Country: item.countryData?.name ?? "N/A",
        ...
        <ComponentCard title="Business List">
          {loading ? (
            ...
            <AdvancedTable
              ...
              type="business"
              enableSearch={true}
              onSearch={(search) => fetchUserBusiness(1, search)}
            />
          )}
        </ComponentCard>
      </div>
