// src/components/tables/AdvancedTables/AdvancedTable.tsx
...
import { AdvancedTableProps } from '../../../Types/AdvancedTable';
...
function AdvancedTable<T extends Record<string, any>>({
  ...
  //   exportRow && filteredData.length > 0
  //     ? filteredData.map(exportRow)
  //     : [];
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {enableSearch ? (
          ...
          <div /> // Empty div to maintain spacing if search is disabled
        )}

        {/* Right: Add Button & Export */}
              ...
              <TableRow>
                {columns.map((column, index) => (
                  <TableCell
                    key={index}
                    isHeader
                    className="px-5 py-3 font-bold text-gray-500 text-left uppercase text-theme-xs dark:text-gray-400"
                  >
                    {column.header}
                  </TableCell>
                ))}
      ...
      {/* Footer: Pagination Summary and Controls */}
          ...
          {pagination ? (
            <>
              ...
              <span className="font-semibold">
                {((currentPageValue - 1) * pagination.limit) + 1}
              </span>{" "}
              to{" "}
              <span className="font-semibold">
                {Math.min(currentPageValue * pagination.limit, pagination.total)}
              </span>{" "}
              of{" "}
